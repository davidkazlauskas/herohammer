...
(defn dota-generate-filter-opponent-question-count
  "Generate hero question count for question and filter."
  [hero question-id filter-id]
  ["dota" "question-opponent-filter-count"
   (str hero "-" question-id "-" filter-id)])

(defn dota-generate-filter-question-top-count
  "Generate hero question count for question and filter."
  [question-id filter-id]
  ["dota" "question-answer-filter-count"
   (str question-id "-" filter-id)])
...
(defn gen-map-reduce-tasks-global [max-proc]
  (let [dota-args
        ...
        {:save-key (dota-generate-most-popular-heroes)
         :final-reduce-function scon/update-most-popular-heroes})
        pop-opponents-args (merge dota-args
        {:save-key (dota-generate-most-popular-opponents)
         :final-reduce-function scon/update-most-popular-opponents})
        ...
        proc-opponents-args (merge dota-args
         {:glob-question-key (dota-generate-opponent-stats-count)
    ...
    [; generic processing
     ...
     ; most popular heroes
     (scon/dummy-final-reduce-proc-job pop-heroes-args)
     ; process most popular opponents
     (scon/dummy-final-reduce-proc-job pop-opponents-args)
     ; process hero counts
...
(def ^:dynamic *hh-context-dota*
  {
   ...
   :queries {
     ...
     :matchup-opponent-count dota-generate-filter-opponent-question-count
     :matchup-question-top-answer dota-generate-filter-question-top-count
     ...
     :opponent-most-popular-global dota-generate-most-popular-opponents
   }
   :questions {
